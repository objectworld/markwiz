...
| `bundle/msi/Markwiz_0.1.2_x64_ko-KR.msi` | MSI 설치 패키지(한국어) |
| `bundle/msi/Markwiz_0.1.2_x64_en-US.msi` | MSI 설치 패키지(영어) |
...
- **한국어 / 영어 지원**: 모든 메뉴, 툴바, 상태바, 대화상자, 예제 문서가 한국어와 영어로 표시됩니다. **도움말 > 언어 / Language**에서
  언제든 바꿀 수 있고 선택은 기억됩니다. NSIS 설치 프로그램은 시작할 때 언어를 고르며(설치 화면도 그 언어로 표시), MSI는
  언어별 파일(`_ko-KR.msi`, `_en-US.msi`)로 나뉩니다. 설치할 때 고른 언어가 앱을 처음 실행할 때의 기본 언어가 됩니다.
  설치하지 않은 경우(웹, 단독 실행)에는 브라우저/OS 언어를 따릅니다
